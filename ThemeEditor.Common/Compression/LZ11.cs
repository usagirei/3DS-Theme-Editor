...
namespace ThemeEditor.Common.Compression
{
    ...
    public class LZ11 // LZ11 (de)compression
    {
        ...
        /// <exception cref="NotEnoughDataException">
        ...
        /// </summary>
        private static unsafe int CompressWithLA(Stream instream, long inLength, Stream outstream)
        {
            // save the input data in an array to prevent having to go back and forth in a file
            byte[] indata = new byte[inLength];
            int numReadBytes = instream.Read(indata, 0, (int) inLength);
